...
# Load image feature vectors and FAISS index
...
async def search_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        for n, i in enumerate(indices[0]):
            ...
            top_n_item = next((item for item in data if item['eng_id'] == top_n_id), None)
            if top_n_item:
                results.append({
                    'rank': n+1,
                    'file': top_n_id,
                    'description': top_n_item['description'],
                    'explanation': top_n_item.get('explanation', 'N/A'),
                    'comment': top_n_item.get('comment', 'N/A'),
                    'meta': top_n_item.get('meta', 'N/A')
                })
